...
class Selection implements \Iterator, IRowContainer, \ArrayAccess, \Countable
{
  ...
  protected $paginator;
	...
	{
        $db_name = explode('=', explode(';', $this->context->getConnection()->getDsn())[1])[1];
        $maxID = $this->context->query("SELECT AUTO_INCREMENT FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_SCHEMA = ? AND TABLE_NAME = ?", $db_name, $this->name)->fetch()['AUTO_INCREMENT'];
        ...
        $primary_key_name = $this->context->query("SHOW KEYS FROM `?` WHERE Key_name = ?", new Nette\Database\SqlLiteral($this->name), 'PRIMARY')->fetch()['Column_name'];
        $row = $this->context->table($this->name)->where($primary_key_name, $maxID)->fetch();
  ...
  public function filter(FilterStructure $filterStructure): Selection {
    $conditionArray = $filterStructure::$filter;
    if (isset($conditionArray['like'])) {
      foreach ($conditionArray['like'] as $key => $value) {
        $this->where($key . ' LIKE ?', '%' . $value . '%');
      }
    }
    if (isset($conditionArray['equal'])) {
      foreach ($conditionArray['equal'] as $key => $value) {
        $this->where($key, $value);
      }
    }
    if (isset($conditionArray['between'])) {
      foreach ($conditionArray['between'] as $key => $value) {
        $this->where('? BETWEEN ? AND ?', new Nette\Database\SqlLiteral($key), $value['from'], $value['to']);
      }
    }

    $this->paginator = new Paginator($this->count(), $filterStructure::$currentPage, $filterStructure::$pageSize);
    if ($filterStructure::$orderBy) {
        $this->order($filterStructure::$orderBy . $filterStructure::$asc);
    }
    $this->limit($this->paginator->getPageSize(), $this->paginator->getStartIndex());
...
class FilterStructure {
  ...
  /**
   * TODO: přidat podmínky pro ověření správné struktury.
   * Metoda pro setnutí filteru
   * @param array $filters
   * @param array $orderBy
   ...
   */
  public static function setFilter(array $filters, array $orderBy, int $currentPage, int $pageSize) {
    foreach ($filters as $key => $filter) {
      self::$filter[$filter['type']][$key] = $filter['value'];
    }

    if (isset($orderBy['column'])) {
        self::$orderBy = $orderBy['column'];
        self::$asc = $orderBy['asc'] ? ' ASC' : ' DESC';
    }
