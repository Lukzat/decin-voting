...
class AddPageManager extends BaseManager {
    ...
    public function quickSearch($info) {
        ...
        $cityId = array_map('iterator_to_array',$this->database->query(
                ...
                .self::TABLE_NAME_3.
                " WHERE "
                .self::COLUMN_NAME_3."='".(string)$cityName."'")->fetchAll())[0]['id'];
        ...
        foreach ($sittings as $sitting) {
            ...
            $SessionsResult = array_map('iterator_to_array',$this->database->query(
                    ...
                    " LIKE '%{$searchWord}%'"
                    . " OR "
                    . self::COLUMN_ABOVE_DESCRIPTION_5.
                    " LIKE '%{$searchWord}%'"
                    . " OR "
                    . self::COLUMN_CHAIRMAN_5.
                    " LIKE '%{$searchWord}%'"
                    . " OR "
                    . self::COLUMN_STATE_5.
                    " LIKE '%{$searchWord}%'"
            ...
            if(sizeof($SessionsResult) > 0){
                $sessionResultsArray[] = $SessionsResult;
            }
        }
        $finalResults[0] = $sessionResultsArray;
        foreach ($sittings as $sitting) {
            $sittingId = $sitting['id'];
            $sessionIds = array_map('iterator_to_array',$this->database->query(
            "SELECT id FROM "
            .self::TABLE_NAME_5.
            " WHERE "
            .self::COLUMN_SITTING_ID_5."=".(int)$sittingId."")->fetchAll());
            foreach ($sessionIds as $sessionId) {
                ...
                $VotedResult = array_map('iterator_to_array',$this->database->query(
                    ...
                    " LIKE '%{$searchWord}%'"
                    . " ) AND ( "
                    . self::COLUMN_SESSION_ID."=".(int)$sessId.")")->fetchAll());
                if(sizeof($VotedResult) > 0){
                    $votedResultsArray[] = $VotedResult;
                }
            }
        }
        if(sizeof($votedResultsArray) > 0){
            $finalResults[1] = $votedResultsArray;
        }
        return $finalResults;
